...
trace. Rule tags often record a ticket or an owner, so they tell you who must
approve a delete. A row with `field: dynamic` shows the tags of the dynamic
address group, not the filter tags that match.
...
object (member tree + effective leaves); the `u` where-used spoke lists each
referrer with its own `tags` column, like the CLI listing; `G` adds the current
selection as
