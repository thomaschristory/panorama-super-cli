...
Each row describes the *referrer*, not the object you trace. The `tags` field
shows the tags of that referrer. The referrer is the rule or the group that
points at the object. Table and CSV output show the field as a `tags` column and
join the tags with commas. JSON, JSONL and YAML output carry a real list. An
untagged referrer leaves the table cell and the CSV field empty. The same
referrer shows an empty list in JSON, JSONL and YAML. A rule tag often records a
ticket, an owner, or an audit scope. Use it to send a delete or a rename to the
correct team. `refs dangling` rows carry the same `tags` field.

Two values of the `field` entry need an explanation. A row with the value
`dynamic` comes from a dynamic address group that matches the object by tag. The
`tags` field shows the tags of that group. It does not show the filter tags
that cause the match. Read the group filter to see those. A row with the value
