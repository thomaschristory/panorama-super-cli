...
def _find_addr(snapshot: Snapshot, name: str, loc: Location) -> Address | None:
    return next(
        (a for a in snapshot.addresses if a.name == name and a.location == loc), None
    )
...
def _find_service(snapshot: Snapshot, name: str, loc: Location) -> Service | None:
    return next(
        (s for s in snapshot.services if s.name == name and s.location == loc), None
    )


def _find_service_group(
    snapshot: Snapshot, name: str, loc: Location
) -> ServiceGroup | None:
    return next(
...
def _expand_addr_members(
    ...
    for m in members:
        ...
        if target is None:
            ...
            nodes.append(
                InspectNode(
                    kind="address", name=m, location=loc.name, status=NodeStatus.DANGLING
                )
            )
            continue
...
def _svc_group_node(
    ...
    children = _expand_svc_members(
        snapshot, graph, sg.location, sg.members, seen | {key}, acc
    )
    return InspectNode(
...
def _expand_svc_members(
    ...
    for m in members:
        ...
        if target is None:
            ...
            nodes.append(
                InspectNode(
                    kind="service", name=m, location=loc.name, status=NodeStatus.DANGLING
                )
            )
            continue
...
def _expand_rule_addr_members(
    ...
    for m in members:
        if m == "any":
            nodes.append(
                InspectNode(kind="address", name="any", location=loc.name, detail="(any)")
            )
            continue
        target = graph.resolve(ADDR_NS, m, loc)
        if target is None:
...
def _expand_rule_svc_members(
    ...
    for m in members:
        if m in ("any", "application-default"):
            nodes.append(
                InspectNode(kind="service", name=m, location=loc.name, detail=f"({m})")
            )
            continue
        target = graph.resolve(SVC_NS, m, loc)
        if target is None:
...
def _rule_view(
    ...
    for label, ns, members in fields:
        ...
        children.append(
            InspectNode(kind="field", name=label, location=loc.name, children=fnodes)
        )
...
def _view_address(a: Address) -> ObjectView:
    ...
    node = InspectNode(
        kind="address", name=a.name, location=a.location.name, detail=a.value
    )
...
def _view_service(s: Service) -> ObjectView:
    ...
    node = InspectNode(
        kind="service", name=s.name, location=s.location.name, detail=disp
    )
    return ObjectView(
        kind="service",
        name=s.name,
...
def _view_address_group(
    snapshot: Snapshot, graph: ReferenceGraph, g: AddressGroup
) -> ObjectView:
    acc = _Acc()
...
def _view_service_group(
    snapshot: Snapshot, graph: ReferenceGraph, sg: ServiceGroup
) -> ObjectView:
    acc = _Acc()
...
def _tag_carriers(snapshot: Snapshot, name: str) -> list[InspectNode]:
    ...
    for r in snapshot.policy_rules:
        if name in r.tags:
            nodes.append(
                InspectNode(kind=r.referrer_kind, name=r.name, location=r.location.name)
            )
    return nodes
...
def _view_security_rule(
    snapshot: Snapshot, graph: ReferenceGraph, r: SecurityRule
) -> ObjectView:
    return _rule_view(
        snapshot,
        graph,
...
def _view_nat_rule(
    snapshot: Snapshot, graph: ReferenceGraph, r: NatRule
) -> ObjectView:
...
def _view_policy_rule(
    snapshot: Snapshot, graph: ReferenceGraph, r: PolicyRule
) -> ObjectView:
    return _rule_view(
        snapshot,
        graph,
